...
# find pane IDs using: `osascript -e 'tell application "System Preferences"'' -e 'get the ID of the current pane' -e 'end tell'`
...
osascript -e 'tell app "System Preferences"' -e 'activate' -e 'set the current pane to pane id "com.apple.Displays-Settings.extension"' -e 'end tell'
...
# Enable snap-to-grid and arrange icons by namem on the desktop and in other icon views
...
# Disable ⌘+⌥+Space shortcut to show Finder search window
...
# Apply changes made to com.apple.symbolichotkeys
