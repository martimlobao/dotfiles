...
def test_uninstall_previous_source_when_no_previous_source(tmp_path: Path) -> None:
    facade, _, _ = build_facade(tmp_path, content='[cli]\nfoo = "formula"\n')
    outcome = app_module.AddAppOutcome(
        app_key="foo",
        group="cli",
    ...
    result = facade._uninstall_previous_source(outcome, "uv")

    assert result is None


def test_uninstall_previous_source_when_same_source(tmp_path: Path) -> None:
    facade, _, _ = build_facade(tmp_path, content='[cli]\nfoo = "formula"\n')
    outcome = app_module.AddAppOutcome(
        app_key="foo",
        group="cli",
    ...
    result = facade._uninstall_previous_source(outcome, "formula")

    assert result is None


def test_install_to_new_source_returns_error_on_exception(tmp_path: Path) -> None:
    facade, _, _ = build_facade(tmp_path, content='[cli]\nfoo = "formula"\n')
    service = Mock()
    service.ensure_installed.side_effect = app_module.AppManagerError("install error")

    with patch.object(facade, "get_service", return_value=service):
        result = facade._install_to_new_source("foo", "uv")

    assert result == "install error"
